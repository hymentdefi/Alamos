...
/* ─── Monte Carlo Simulation (Fase 3 spec) ──────────────────────
 *
 * Genera miles de trayectorias estocásticas del portfolio hacia
 * el futuro basadas en geometric Brownian motion (GBM) — el modelo
 * estándar de log-normal return diffusion.
 *
 * Modelo por mes:
 *   V[t+1] = V[t] × exp((μ - σ²/2)·Δt + σ·√Δt·Z) + aporte_mensual
 *
 * Donde:
 *   μ = retorno esperado anualizado (decimal)
 *   σ = volatilidad anualizada (decimal)
 *   Δt = 1/12 (paso mensual)
 *   Z = N(0,1) random
 *
 * Spec 5.4 pide fan chart con percentiles P10/P25/P50/P75/P90 por
 * mes. Acá los exportamos como MonteCarloPoint[].
 *
 * Seedeado para reproducibilidad — mismo seed da mismo resultado.
 * En prod este cálculo va server-side (Python + NumPy, 5000 iter).
 * Para mock client-side hacemos 1000 iter (balance entre precisión
 * y velocidad mobile).
 */

export interface MonteCarloPoint {
  /** Mes 0-indexed (0 = ahora, N = horizonte). */
  month: number;
  p10: number;
  p25: number;
  p50: number;
...
export interface MonteCarloResult {
  ...
  /** Array de length = horizonYears * 12 + 1 (incluye t=0). */
  points: MonteCarloPoint[];
  /** % de iteraciones que terminaron ≥ targetValue. Solo si se
   *  pasó target en el input. */
  probabilityOfTarget?: number;
  /** Valor final P50 (mediana de la trayectoria final). */
  medianFinal: number;
  /** Valor final P10 (escenario pesimista). */
  pessimisticFinal: number;
  /** Valor final P90 (escenario optimista). */
  optimisticFinal: number;
}

interface SimulateArgs {
  /** Valor actual del portfolio en moneda de display. */
  initialValue: number;
  /** Horizonte en años (default 10). */
  horizonYears?: number;
  /** Aporte mensual recurrente en moneda de display. */
  monthlyContribution?: number;
  /** Retorno esperado anualizado (%, ej 12 = 12% anual). */
  annualReturn: number;
  /** Volatilidad anualizada (%, ej 15 = 15% anual). */
  annualVol: number;
  /** Meta opcional para computar probabilidad de alcanzarla. */
  targetValue?: number;
  /** Iteraciones (default 1000 client-side). */
  iterations?: number;
  /** Seed determinístico para reproducibilidad. */
  seed?: string;
}

/* PRNG xorshift-style seedeado desde un string. Cada call devuelve
 * un float en [0, 1). Necesario para que el fan chart sea estable
 * entre re-renders (sino el chart "tiembla" con cada update). */
...
/* Box-Muller transform: dos uniform → un normal N(0,1). */
...
export function simulateMonteCarlo(args: SimulateArgs): MonteCarloResult {
  const {
    initialValue,
    horizonYears = 10,
  ...
  /* Buckets de valores por mes — usados para extraer percentiles
   * al final. Length = months + 1 para incluir t=0. */
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    for (let t = 1; t <= months; t++) {
      ...
      V = V * Math.exp(drift * dt + diffusion * z) + monthlyContribution;
      buckets[t].push(V);
    }
  }

  const points: MonteCarloPoint[] = buckets.map((values, month) => {
    values.sort((a, b) => a - b);
    const pct = (p: number) =>
      values[Math.max(0, Math.floor((values.length - 1) * p))];
    return {
      month,
      p10: pct(0.1),
      p25: pct(0.25),
  ...
  let probabilityOfTarget: number | undefined;
  if (targetValue != null && targetValue > 0) {
    const finalValues = buckets[months];
    const hits = finalValues.filter((v) => v >= targetValue).length;
    probabilityOfTarget = (hits / finalValues.length) * 100;
  }
  ...
  return {
    ...
    points,
    probabilityOfTarget,
    medianFinal: final.p50,
    pessimisticFinal: final.p10,
    optimisticFinal: final.p90,
  };
}
